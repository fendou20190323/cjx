package com.lss.common.util.excel;
...
public class ExcelUtil {
     ...
     * @Param special     是否存在特殊处理行，默认false
     */
    public static <T> Collection<T> importExcel(Class<T> clazz, File file, Boolean special,
                                                String pattern, ExcelLogs logs, Integer... arrayCount) throws IOException {
        ...
        try {
            ...
            Map<Integer, Object> specialIndexMap = new HashMap<>();
                    ...
                    if (row.getRowNum() == 0) {
                        if (clazz == Map.class) {
                            ...
                            while (cellIterator.hasNext()) {
                                ...
                                if (index % 2 == 0) {
                                    int num = index - 1;
                                    specialMap.put(specialIndexMap.get(num), cellValue);
                                } else {
                                    specialIndexMap.put(index, cellValue);
                                }
                                index++;
                            }
                        }
                        list.add((T) specialMap);
                        continue;
                    }
                    if (row.getRowNum() == 1) {
                        if (clazz == Map.class) {
    ...
    public static <T> Collection<T> importExcel(Class<T> clazz, InputStream inputStream, Boolean special,
                                                String pattern, ExcelLogs logs, Integer rowLimit, Integer... arrayCount) throws IOException {
        Workbook workBook;
        try {
            workBook = WorkbookFactory.create(inputStream);
        } catch (Exception e) {
        ...
        if (sheet.getLastRowNum() > rowLimit) {
            throw new RuntimeException("已经超过最大行数:" +  rowLimit + "行");
        }
        Iterator<Row> rowIterator = sheet.rowIterator();
        try {
            List<ExcelLog> logList = new ArrayList<>();
