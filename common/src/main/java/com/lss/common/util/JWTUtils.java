...
public class JWTUtils {
	...
	public JWTResult checkToken(String token) {
		try {
			...
			return new JWTResult(true, sub, "合法请求", Result.ResultCode.SUCCESS.code);
		} catch (ExpiredJwtException e) {
			// 在解析JWT字符串时，如果‘过期时间字段’已经早于当前时间，将会抛出ExpiredJwtException异常，说明本次请求已经失效
			return new JWTResult(false, null, "token已过期", Result.ResultCode.TOKEN_TIMEOUT_CODE.code);
		} catch (SignatureException e) {
			// 在解析JWT字符串时，如果密钥不正确，将会解析失败，抛出SignatureException异常，说明该JWT字符串是伪造的
			return new JWTResult(false, null, "非法请求", Result.ResultCode.UNAUTHORIZED.code);
		} catch (Exception e) {
			return new JWTResult(false, null, "非法请求", Result.ResultCode.UNAUTHORIZED.code);
		}
	}
	 ...
	 * @return JWTResult
	 */
	public JSONObject getTokenSub(String token) {
		try {
			Claims claims = Jwts.parser().setSigningKey(pubKey).parseClaimsJws(token).getBody();
			return JSONObject.parseObject(claims.get("sub", String.class));
		} catch (Exception e) {
	 ...
	 * @return JWTResult
	 */
	public JSONObject getTokenSubForException(String token) throws Exception{
		Claims claims = Jwts.parser().setSigningKey(pubKey).parseClaimsJws(token).getBody();
		return JSONObject.parseObject(claims.get("sub", String.class));
	}

	public static class JWTResult implements Serializable {
